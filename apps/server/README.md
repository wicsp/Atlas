...
This directory contains the Atlas backend for system status, internal service probes, content
review, and agent coordination. It is designed for access through a private network or Tailscale,
with application authentication and agent/service tokens as additional protection layers.
...
The backend does not serve frontend assets and does not replace SSH, rsync, Syncthing, MinIO/S3,
Tailscale, Docker, or systemd. The separately deployed Console lives in `../console`; the backend's
job is to coordinate, persist, and expose state through documented APIs.

## System documentation

Atlas is one part of a personal infrastructure system:
...
Cross-system development should follow an accepted RFC and proceed in this order: Atlas protocol,
server and Console implementation, Lumio execution, `nix-config` deployment, then one end-to-end
verification.

Atlas stores Source/Resource provenance, KnowledgeRefs, and explicitly completed synchronized
Comments. Transcript, summary, PDF, and experiment bytes remain external ArtifactRefs. Obsidian is
the local writing plane; only an explicit completion workflow uploads human comment prose.
...
```bash
export ATLAS_AGENT_AUTHORIZATION='Authorization: replace-with-scheme-and-token'
curl -X POST http://127.0.0.1:8000/api/agents/register \
  -H "$ATLAS_AGENT_AUTHORIZATION" \
